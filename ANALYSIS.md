...
3.  **多路聚合轮询 (Poller)**：
    *   **物理隔离，逻辑统一**：行情数据（Market Journal）和交易回报（Trade Journal）物理上存储于不同的共享内存块中，写入互不干扰。
    *   **消费端归并**：消费者（如 Strategy Runner）通过 `Poller` 在单线程内极速轮询这些独立的通道。`Poller` 将来自不同源的消息按接收顺序（或时间戳）“逻辑合并”为一个单一的事件流。
    *   **开发体验**：策略开发者无需编写复杂的多线程同步代码，只需在统一的 `on_tick` / `on_response` 回调中处理有序到达的各类事件。
