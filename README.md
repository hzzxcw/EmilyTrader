...
**⚠️ 重要提醒：当前 OrderBook 实现为模拟用途**

当前的 `OrderBook` 类在 `include/orderbook/orderbook.h` 中的生成逻辑仅用于演示和模拟目的。在实际生产环境中，需要特别注意以下差异：

### 当前模拟实现的特点：
-   **混合处理**：将委托（Entrust）和成交（Execution）统一处理
-   **简化逻辑**：根据 `bid_no`/`ask_no` 直接移除订单，未区分委托类型
-   **无撤单处理**：当前模拟行情数据中没有包含撤单（Cancel）操作的定义

### 实际交易系统需要考虑：
1.  **委托类型识别**：区分新委托、修改委托、撤单委托
2.  **订单状态管理**：跟踪订单生命周期（已提交、部分成交、全部成交、已撤单）
3.  **价格时间优先级**：严格遵循价格优先、时间优先的撮合规则
4.  **市场深度数据**：支持 L1/L2/L3 不同深度的市场数据推送

### 模拟数据的限制：
当前的行情模拟器（`apps/simulator`）生成的 `Entrust` 和 `Execution` 数据是为了测试系统性能而设计的简化模型。在真实的中国A股交易环境中，行情数据会更加复杂，包含更多的订单状态变化和撤单信息。
